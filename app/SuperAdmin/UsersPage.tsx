...
import { loadUsers as loadADUsers } from "./utils/adUsers";
...
function getRoleStyle(role: string): { bg: string; text: string; label: string } {
  switch (role) {
    case "superadmin": return { bg: "#1e3a5f", text: "#93c5fd", label: "Super Admin" };
    case "admin":      return { bg: "#3b1f5e", text: "#d8b4fe", label: "Admin" };
    default:           return { bg: "#2d3748", text: "#cbd5e0", label: "Employee" };
  }
}
...
export default function UsersPage({ currentUser }: Props) {
  ...
  const [roleFilter, setRoleFilter] = useState<"all" | "superadmin" | "admin" | "employee">("all");
  ...
  async function fetchUsers(forceSync = false) {
    ...
    try {
      ...
      console.log(synced ? "✅ Synced from AD" : "⚡ Loaded from Firestore cache");
  ...
  if (error) {
    return (
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 24, backgroundColor: theme.background }}>
        ...
        <TouchableOpacity
          onPress={() => fetchUsers()}
          style={{ backgroundColor: theme.iconActive, borderRadius: 10, paddingHorizontal: 20, paddingVertical: 10 }}
        >
    ...
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      <View style={{
        ...
        <View>
          ...
          <Text style={{ color: theme.subtext, fontSize: 12, marginTop: 2 }}>
            {lastSynced ? `Last synced from AD · ${lastSynced}` : "Loaded from cache · ocgbim.com"}
          </Text>
        </View>
      ...
      <View style={{ flexDirection: "row", gap: 10, paddingHorizontal: 20, marginBottom: 16 }}>
        {[
          ...
          >
            <Text style={{ color: theme.subtext, fontSize: 10, marginBottom: 4 }}>
              {stat.label}
            </Text>
        ...
        {roleTabs.map((tab) => (
          <TouchableOpacity
            ...
            style={{
              ...
              paddingVertical: 6,
              borderRadius: 8,
              backgroundColor: roleFilter === tab.key ? theme.iconActive : theme.surface,
              borderWidth: 1,
              borderColor: roleFilter === tab.key ? theme.iconActive : theme.border,
            }}
          >
            <Text
              style={{
      ...
      {filtered.length === 0 ? (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
          <Text style={{ color: theme.subtext, fontSize: 13 }}>No users found.</Text>
        </View>
      ) : (
        <FlatList
          data={filtered}
          ...
          renderItem={({ item }) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Text style={{ color: role.text, fontSize: 13, fontWeight: "700" }}>
                ...
                <View style={{ alignItems: "flex-end", gap: 4 }}>
                  <View style={{ backgroundColor: role.bg, borderRadius: 99, paddingHorizontal: 8, paddingVertical: 3 }}>
                    <Text style={{ color: role.text, fontSize: 10, fontWeight: "600" }}>
                      {role.label}
                    </Text>
                  </View>
                  <View
                    style={{
                      backgroundColor: item.hasLoggedIn ? "#052e16" : theme.bgActive,
                      borderRadius: 99,
                      paddingHorizontal: 8,
                      paddingVertical: 3,
                    }}
